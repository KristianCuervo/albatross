"""
albatross.macroscale.hull — velocity hull: the microscale → macroscale bridge.

VelocityHull encapsulates the convex hull of achievable average velocities for
all V_ref levels.  It is built from a microscale Ensemble (or loaded from a
precomputed NPZ) and queried by GreedyMigration to convert ERA5 wind at each
grid point into a reachable velocity set.

Coordinate conventions (consistent with CG2/migration_hamiltonian.py)
----------------------------------------------------------------------
Wind frame:
...
class VelocityHull:
    ...
    def from_ensemble(cls, ensemble) -> "VelocityHull":
        """
        Build hull from a live microscale Ensemble object.
        ...
        """
        from ..microscale.ensemble import Ensemble
        ...
        return cls(
            ...
            hull_radii   = np.array(hull_radii_list),
    ...
    def as_dict(self) -> dict:
        """Return hull data as a plain dict (compatible with migration_hamiltonian)."""
        return {
            "v_ref_levels": self.v_ref_levels,
            "hull_radii":   self.hull_radii,
            "hull_angles":  self.hull_angles,
    ...
    def to_geographic(self, u10: np.ndarray, v10: np.ndarray) -> dict:
        ...
        For each grid point, interpolates the hull to the local V_ref and
        resamples it onto a fixed geographic bearing grid (0 = North, CW).
        ...
        -------
        dict with keys:
            v_ref_local          (n_lat, n_lon)
            wind_dir             (n_lat, n_lon)   bearing wind blows TO, CW from N [rad]
            upwind_bearing       (n_lat, n_lon)   bearing wind comes FROM, CW from N [rad]
            max_speed_achievable (n_lat, n_lon)
            hull_radii_geo       (n_lat, n_lon, n_rays)
            geo_bearings         (n_rays,)
        """
        ...
        hull_interp = self._interpolate_radii(v_ref_clamped)  # (n_pts, N_RAYS)
